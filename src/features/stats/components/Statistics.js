...
function Statistics() {
      ...
      .then((data) => {
        const safeOrders = data.map((o) => ({
          ...
          dateOfOrder: o.dateOfOrder
            ? new Date(o.dateOfOrder).toISOString()
            : null,
          total: o.total || 0,
        }));
  ...
  orders.forEach((order) => {
    const prod =
      order.product || products.find((p) => p.id === order.productId);
    if (!prod) return;

    if (!productSales[prod.id])
      productSales[prod.id] = { name: prod.name, quantity: 0 };
  ...
  const generateShades = (baseColor, count) => {
    ...
    for (let i = 0; i < count; i++) {
      ...
      const color = baseColor
        .match(/\w\w/g)
        .map((c) =>
          Math.round(parseInt(c, 16) * factor)
            .toString(16)
            .padStart(2, "0")
        )
        .join("");
    ...
    <div className="dashboard-container">
      ...
      >
        ...
        <div className="dashboard-subtitle">
          Overview of your products and orders (last 30 days)
        </div>
        ...
        <div className="stats-cards">
          ...
          <motion.div className="stat-card" whileHover={{ scale: 1.05 }}>
            ...
            <p>{orders.reduce((sum, order) => sum + (order.amount || 0), 0)}</p>
          </motion.div>
